...
export function Stats() {
  ...
  const stats = useMemo(() => aggregateStats({ tournaments, aliases }), [tournaments, aliases, lang])
  ...
  return (
    <div className="space-y-8">
      <section className="sticker-lg relative overflow-hidden bg-paper-100 p-6 sm:p-8">
        ...
        <Chip tone="bg-sky text-paper-100">{t('BY THE NUMBERS', 'У ЦИФРАХ')}</Chip>
        <h1 className="mt-3 text-4xl font-extrabold sm:text-5xl">{t('The Big Picture', 'Загальна картина')}</h1>
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <Stat label={t('Tournaments', 'Турніри')} value={stats.tournaments} tone="bg-sky-soft" />
          <Stat label={t('Matches', 'Матчі')} value={stats.matches} tone="bg-mint-soft" />
          <Stat label={t('Hours on court', 'Годин на корті')} value={t(`${stats.hours}h`, `${stats.hours} год`)} sub={t('≈ 2h per event', '≈ 2 год на турнір')} tone="bg-tang-soft" />
          <Stat label={t('Players', 'Гравці')} value={stats.players} tone="bg-punch-soft" />
        </div>
      </section>

      <section>
        <SectionTitle emoji="✨" title={t('Highlights', 'Найцікавіше')} hint={t('Fun facts from the whole history', 'Цікаві факти за всю історію')} />
      ...
      <section>
        <SectionTitle emoji="💸" title={t('The Damage', 'Витрати')} hint={t('Entry fees, roughly — ~$14 / 225k IDR per event', 'Орієнтовні внески — ~$14 / 225k IDR за турнір')} />
        ...
        <div className="mt-6 sticker-lg bg-paper-100 p-5 sm:p-6">
          <div className="flex items-baseline justify-between gap-3">
            <h3 className="font-display text-xl font-extrabold sm:text-2xl">{t('…or, instead 🛒', '…або, натомість 🛒')}</h3>
            <span className="text-xs font-bold uppercase tracking-wider text-ink-faint">
              {t('same money, more fun', 'ті самі гроші, більше фану')}
            </span>
          </div>
